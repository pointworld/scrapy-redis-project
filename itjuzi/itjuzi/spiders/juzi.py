...
class ITjuziSpider(RedisCrawlSpider):
    ...
    def parse_item(self, response):
        global company_name, slogan, scope, sub_scope, city, area, home_page
        global tags, company_intro, company_full_name, found_time, company_status
        ...
        item = CompanyItem()
        item['id'] = response.url.split('/')[-1:][0]
        item['name'] = company_name
        item['slogan'] = slogan
        item['category'] = scope
        item['sub_categroy'] = sub_scope
        item['city'] = city
        item['area'] = area
        item['homepage'] = home_page
        item['tags'] = tags
        item['breif'] = company_intro
        item['fullname'] = company_full_name
        item['found_time'] = found_time
        item['company_status'] = company_status
        item['investment_info'] = tz_list
        item['employee_info'] = tm_list
        item['product_info'] = pdt_list
        return item
